...
export class PruebaService {
  ...
  getAllPruebas(): Observable<any> {
    const url = 'http://127.0.0.1:8000/api/prueba/';
    return this.http.get<any>(url);
  }

  getPruebaInfo(idPrueba: string): Observable<any> {
    const url = 'http://127.0.0.1:8000/api/prueba/' + idPrueba + '/';
    return this.http.get<any>(url);
  }
  ...
  buscarPorLongLat(direccion : string): Observable<any>{
    const url = 'http://127.0.0.1:8000/api/prueba/busquedaLatLot/' + direccion + '/';
    return this.http.get<any>(url);
  }
